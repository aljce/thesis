...
% TODO: citation going off margin
...
\begin{align}
  ...
  \end{split}
\end{align}
Note that at every level of recursion we only need to inspect the last element
of the bitstring. Then we drop that element. Thus if we store the bitstring as a
snoc list we can perform the required operations in $\BigO{1}$. A snoc list is a
just a linked list where a user can append/uncons from the back of the list in
$\BigO{1}$ time \cite{okasaki}.
\begin{code}
  infixl 5 _0ᵇ _1ᵇ
  data 𝔹⁺ : Set where
    𝕓1ᵇ : 𝔹⁺
